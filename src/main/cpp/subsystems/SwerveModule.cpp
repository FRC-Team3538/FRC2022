...
{
  // Real Hardware
  return 0_mps; //m_driveMotor.GetSelectedSensorVelocity(0) * kDriveScaleFactor / 100_ms;
}
...
void SwerveModule::SetModule(const frc::SwerveModuleState &state)
{
  ...
  m_driveVolts = 1_V; //units::volt_t{driveOutput} + driveFeedforward;
  ...
  m_turnVolts = 1_V; //units::volt_t{turnOutput} + turnFeedforward;
