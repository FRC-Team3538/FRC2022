...
package edu.wpi.first.y2023.math.filter;

import edu.wpi.first.util.CircularBuffer;
import edu.wpi.first.y2023.math.MathSharedStore;
import edu.wpi.first.y2023.math.MathUsageId;
...
public class LinearFilter {
  ...
  public static LinearFilter finiteDifference(int derivative, int[] stencil, double period) {
    ...
    //
    // [s<sub>1</sub><sup>0</sup>   ⋯  s<sub>n</sub><sup>0</sup> ][a<sub>1</sub>]      [
    // δ<sub>0</sub>,d ]
    // [ ⋮    ⋱  ⋮   ][⋮ ] = d! [  ⋮   ]
    // [s<sub>1</sub><sup>n-1<sup> ⋯ s<sub>n</sub><sup>n-1<sup>][a<sub>n</sub>]
    // [δ<sub>n</sub>₋<sub>1</sub>,d]
    //
    // where δᵢ,ⱼ are the Kronecker delta. The FIR gains are the elements of the
    // vector a in reverse order divided by hᵈ.
    //
    // The order of accuracy of the approximation is of the form O(h<sup>n-d<sup>).
